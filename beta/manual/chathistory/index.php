...
if(isset($_GET['chat'])) {
	...
	if(mysql_num_rows($get_chat) > 0) {
		while ($chat = mysql_fetch_assoc($get_chat)) {
			...
			if($chat['channel'] == "Login" || $chat['channel'] == "Forum" || $chat['channel'] ==
				"Staff Forum") {
	...
	$sql = mysql_query("SELECT * FROM `chat` WHERE `channel` != \"w\" AND `channel` != \"Staff\"  AND `channel` != \"Staff Forum\" AND `posttime`>=" .
		(time() - $time));
