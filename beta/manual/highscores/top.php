...
for ($skill = 0; $skill < count($skill_info); $skill++) {
    ...
    $get_player_info = mysqli_query($link, "SELECT `player_id`, `exp` FROM `player_skill` WHERE `skill_name`=\"".$skill_info[$skill]['skill_name']."\" ORDER BY `exp` DESC LIMIT 0,10");
    $player_info = mysqli_fetch_all($get_player_info, MYSQLI_ASSOC);
    
	for ($player = 0; $player < 10; $player++) {
	   $get_username = mysqli_query($link, "SELECT `username` FROM `users` WHERE `user_id`=".$player_info[$player]['player_id']);
