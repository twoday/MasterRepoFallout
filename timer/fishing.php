...
$skillc = mysql_query("SELECT `level`,`exp` FROM `player_skill` WHERE `player_id` = '$userinfo[user_id]' && `skill_name` = 'Fishing' ") or die('Error');
...
mysql_query("UPDATE `users` SET `action_skill` = 'Fishing' WHERE `user_id` = '$userid'");
...
var gained = "<b>Sadly you failed to catch any <?php echo $item_name; ?>.</b><br>";
...
var gained = "<b>You managed to catch one <?php echo $item_name; ?>.</b><br>";
var number = "1";
...
function other2(id,fail_exp){
        ...
        $.post("everything.php", { p:30, aid:id, exp:fail_exp },
			function(data) {
				eval(data);
			});
...
function other(id,num_ore){
        ...
        $.post("everything.php", { p:31, aid:id },
			function(data) {
				eval(data);
			});
...
document.getElementById("fishing_exp2").innerHTML = total2;

var total7 = atotal + " Remaining Exp";
document.getElementById("fishing_exp3").innerHTML = total7;
...
document.getElementById("fishing_level").innerHTML = exc;
update_menu(<?php echo $player_location; ?>);
subExp2(exc,exp_b);

var message = "You have advanced a Fishing level (" + exc + ").";
...
document.getElementById("fishing_exp3").innerHTML = total2;
...
<span class="other" id="gained_item"><b>You start fishing for <?php echo "$item_name"; ?> in the cold lake.</b><br></span>
...
<span class="other">Fishing Level: <span id="level"><?php echo "$level"; ?></span> [ <span id="exp"><?php echo "$exp"; ?></span> Exp / <span id="remaining"><?php echo "$remaining"; ?></span> Remaining Exp ]
...
if($players_number == 0){
echo "<br>You are fishing alone.";
}else{
echo "<br>You are fishing along with $players_number others.";
}
?>
</span>
<?php
