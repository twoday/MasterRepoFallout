...
$check1 = mysql_query("SELECT * FROM `guild_members` WHERE `userid` = '$userid' LIMIT 1") or
	die('Error');
...
$level = $have['level'];
$exp = $have['exp'];
$thexp1 = calculateNextLevel($level, $exp);

$check2 = mysql_query("SELECT * FROM `users` WHERE `user_id` = '$have[leader]' LIMIT 1") or
	die('Error');
...
<?php
		...
		while ($row = mysql_fetch_array($result)) {
			...
			while ($row2 = mysql_fetch_array($result2)) {
				...
				$location_check = mysql_query("SELECT * FROM `locations` WHERE `id` = '$row2[location_id]' LIMIT 1") or
					die('Error');
