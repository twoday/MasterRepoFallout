...
while ($actions = mysqli_fetch_assoc($select_actions)) {
	...
	echo "    <td><a class=\"manlinks\" href=\"../locations/" . $location['name'] .
		".php\">" . $location['name'] . "</a></td>";
...
$select_actions = mysqli_query($link,
	"SELECT * FROM `actions` WHERE `name` LIKE 'Prepare %' AND `skill_id` = \"1\" ORDER BY `level` ASC");
while ($actions = mysqli_fetch_assoc($select_actions)) {
	$select_drops = mysqli_query($link,
		"SELECT * FROM `action_drops` WHERE `action_id` = " . $actions['id']);
	echo "<tr>";
...
?>
				...
				>
				<a href="mining.php" class="manlinks">Mining</a>
			</div>
			<footer>
				<br>
				<center>
