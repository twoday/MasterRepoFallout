...
if(mysql_num_rows($get_chat) != 0) {
	while ($chat = mysql_fetch_assoc($get_chat)) {
		...
		$tag = $chat['tag'];
		$msg = chatemote($chat['message'], $userid);
		...
		if(substr($msg, 0, 3) == "/me") {
			...
			if(mysql_num_rows($query) != 0) {
				...
				if($channel == "Admin" || $channel == "Mod") {
					...
					$postername = $row1['staff_name'];
				}
				$msg = "*" . $postername . $message . "*";
		...
		"Staff Forum") {
			...
			$post = "($posttime) " . $msg;
		} elseif($channel == "Guide"){
		  $post = "($posttime) $postername [Guide]: " . $msg;
		} else {
			if($tag) {
		...
		if($channel == "w" && $chats['whispers'] == 1) {
			if($ignored == false) {
				if($posterid == $userid) {
					...
					$chatlog .= "<span style='font-size:13px; color:#6DA6BC;'>($posttime) [w] <span style=\"cursor:pointer; font-size:13px; color:#6DA6BC;\" onclick=\"Tie.whisperPlayer('$postername');\">$postername:</span> $msg </span><br />";
				} else
...
if($chatlog != "") {
	$chatlog = str_replace("+","%20",urlencode($chatlog));
