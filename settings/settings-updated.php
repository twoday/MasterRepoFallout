 ...
 * I'd have the database connection info in a seperate file and just include it in the files that need it rather than having the same code on every page
 ...
 * Added the sql for inserting the old and new usernames into a new table
 */ 
if(isset($_SESSION)){
    if(isset($_POST)){
        ...
        if(isset($_POST['submit_username'])){            
            ...
            if($length < 3){
                ...
                if(mysqli_query($link, $update_sql)){
                    $select_sql = "SELECT `username FROM `users` WHERE `id`=\"".$_SESSION['id']."\"";
                    $row = mysqli_fetch_assoc(mysqli_query($link, $select_sql));
                    $old_username = $row['username'];
                    $insert_sql = "INSERT INTO `usernames` (`old_username`, `new_username`) VALUES (\"".$old_username."\", \"".$new_username."\")";
                    
                    if(mysqli_query($link, $insert_sql)){
                        echo "Username updated";
                        header("Refresh: 2; url='settings.php'");    
                    }else {
                        die("Your username has been changed but a record of the change could not be created. Error: ".mysqli_error($link));
                    }
                }else {
                    ...
                    <form action="settings.php" method="post">
                        <table class="dtab">
                            ...
                            <tr>
                                ...
                                <td><input class="fdi" name="username" size="22" title="Username" <?php if(isset($_POST['submit_username'])){ echo "value=\"".$_POST['username']."\""; } ?> /></td>
                                <td>Costs $3 (Per)</td>
                            ...
                            <tr>
                                ...
                                <td><input type="password" class="fdi" name="password" size="22" title="Password" /></td>
